    ...
    var Particle = /** @class */ (function () {
        function Particle(opacity, particles, color, position) {
                ...
                for (var i = 0; i < _this.particles.settings.particles.array.length; i++) {
                    ...
                    if (dist <= _this.radius + p2.radius) {
                        _this.x = position != null
                            ? position.x
                            : randomFloat() * _this.particles.canvasWidth;
                        _this.y = position != null
                            ? position.y
                            : randomFloat() * _this.particles.canvasHeight;
                        _this.checkOverlap();
                    }
                }
            };
            ...
            if (Array.isArray(color)) {
                var color_selected = color[Math.floor(randomFloat() * this.particles.settings.particles.color.value.length)];
    ...
    var Particles = /** @class */ (function () {
        function Particles(id, config) {
            ...
            this.start = function () {
                _this.createCanvas();
                if (_this.config) {
                    _this.settings = deepExtend(_this.settings, _this.config);
                }
            ...
            this.particlesCreate = function () {
                for (var i = 0; i < _this.settings.particles.number.value; i++) {
                    _this.settings.particles.array.push(new Particle(_this.settings.particles.opacity.value, _this, _this.settings.particles.color.value));
                }
            ...
            this.particlesUpdate = function () {
                for (var i = 0; i < _this.settings.particles.array.length; i++) {
                    ...
                    if (p.x - p.radius > _this.canvasWidth) {
                        p.x = new_pos.x_left;
                        p.y = randomFloat() * _this.canvasHeight;
                    }
                    ...
                    if (p.y - p.radius > _this.canvasHeight) {
                        p.y = new_pos.y_top;
                        p.x = randomFloat() * _this.canvasWidth;
                    }
                    ...
                    if (isInArray('grab', _this.settings.interactivity.events.onhover.mode)) {
                        _this.grabParticle(p);
                    }
                    if (isInArray('bubble', _this.settings.interactivity.events.onhover.mode) || isInArray('bubble', _this.settings.interactivity.events.onclick.mode)) {
                        _this.bubbleParticle(p);
                    }
                    if (isInArray('repulse', _this.settings.interactivity.events.onhover.mode) || isInArray('repulse', _this.settings.interactivity.events.onclick.mode)) {
            ...
            this.particlesRefresh = function (config) {
                ...
                _this.canvasClear();
                if (config) {
                    _this.settings = deepExtend(_this.settings, config);
                }
                /* restart */
                _this.begin();
            };
            ...
            this.pushParticles = function (nb, pos) {
                ...
                for (var i = 0; i < nb; i++) {
                    _this.settings.particles.array.push(new Particle(_this.settings.particles.opacity.value, _this, _this.settings.particles.color.value, {
                        'x': pos != null ? pos.pos_x : randomFloat() * _this.canvasWidth,
                        'y': pos != null ? pos.pos_y : randomFloat() * _this.canvasHeight
                    }));
                    if (i == nb - 1) {
                        if (!_this.settings.particles.move.enable) {
                            _this.particlesDraw();
            ...
            this.bubbleParticle = function (p) {
                ...
                if (_this.settings.interactivity.events.onhover.enable &&
                    isInArray('bubble', _this.settings.interactivity.events.onhover.mode) &&
                ...
                else if (_this.settings.interactivity.events.onclick.enable && isInArray('bubble', _this.settings.interactivity.events.onclick.mode)) {
            ...
            this.repulseParticle = function (p) {
                if (_this.settings.interactivity.events.onhover.enable &&
                    isInArray('repulse', _this.settings.interactivity.events.onhover.mode) &&
                    _this.settings.interactivity.status == 'mousemove') {
                    ...
                    var repulseRadius = _this.settings.interactivity.modes.repulse.distance, velocity = _this.settings.interactivity.modes.repulse.strength, repulseFactor = clamp((1 / repulseRadius) * (-1 * Math.pow(dist_mouse / repulseRadius, 2) + 1) * repulseRadius * velocity, 0, 50);
                    var pos = {
                    ...
                    isInArray('repulse', _this.settings.interactivity.events.onclick.mode)) {
            ...
            this.grabParticle = function (p) {
                if (_this.settings.interactivity.events.onhover.enable &&
                    isInArray('grab', _this.settings.interactivity.events.onhover.mode) &&
                    _this.settings.interactivity.status == 'mousemove') {
                        ...
                        if (opacity_line > 0) {
                            ...
                            if (_this.settings.interactivity.modes.grab.outer_shape.enable) {
                                ...
                                if (_this.settings.interactivity.modes.grab.outer_shape.stroke.color != 'inherit') {
                                    var color = hexToRgb(_this.settings.interactivity.modes.grab.outer_shape.stroke.color);
            ...
            this.eventsListeners = function () {
                ...
                if (_this.settings.interactivity.events.onclick.enable) {
                    _this.settings.interactivity.el.addEventListener('click', function (e) {
                        ...
                        if (_this.settings.interactivity.events.onclick.enable) {
                            if (isInArray('push', _this.settings.interactivity.events.onclick.mode)) {
                                if (_this.settings.particles.move.enable) {
                            ...
                            if (isInArray('remove', _this.settings.interactivity.events.onclick.mode)) {
                                _this.removeParticles(_this.settings.interactivity.modes.remove.particles_nb);
                            }
                            if (isInArray('bubble', _this.settings.interactivity.events.onclick.mode)) {
                                _this.settings.tmp.bubble_clicking = true;
                            }
                            if (isInArray('repulse', _this.settings.interactivity.events.onclick.mode)) {
            ...
            this.init = function () {
                _this.settings.particles.line_linked.color_rgb_line = hexToRgb(_this.settings.particles.line_linked.color);
            ...
            this.begin = function () {
                if (isInArray('image', _this.settings.particles.shape.type)) {
